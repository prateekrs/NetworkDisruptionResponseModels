...
def create_network():
    G=nx.DiGraph()
    network_data=pd.read_csv("F:\\PhD Work\\Disruption network\\Example_network.csv")
#    print network_data.columns

    nodes=list(set(list(network_data['Node1'])+list(network_data['Node2'])))
    node_demands={1:-25,2:0,3:0,4:0,5:25}
...
def disruption_optimization(epsilon_step):
    model=cplex.Cplex()
...
for t in range(0,11):
    disruption_optimization(t/100.000)
